...
**Layout clichés** — no lone centered card (except auth/error/404); no centered hero + floating mockup unless brief says "landing"; no bento grids, three-column icon+title+desc feature grids, or 3-tier pricing unless asked; no generic dashboard (uniform rounded stat tiles, every number same weight); no sticky-everything; no cookie-cutter Problem→Solution→Features→Testimonials→FAQ→CTA or Linear/Vercel/Framer clone.
